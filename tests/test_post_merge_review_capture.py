...
class PostMergeReviewCaptureTest(unittest.TestCase):
    def run_capture(self, *, merged=True, author='owner', reviewer='chatgpt-codex-connector[bot]', submitted='2026-09-05T12:01:00Z', existing='', api_fail=False, lookup_fail=False):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            fixture = {'merged': merged, 'user': {'login': author}, 'title': body,
                       'html_url': 'https://github.com/owner/repo/pull/12', 'merged_at': '2026-09-05T12:00:00Z'}
            ...
            '''))
            gh.chmod(0o755)
            result = subprocess.run(['bash', '-e', '-o', 'pipefail', '-c', shell], env={
                'PATH': str(root) + os.pathsep + os.environ['PATH'], 'FIXTURE_ROOT': str(root),
                'REPO': 'owner/repo', 'PR_NUMBER': '12', 'REVIEWER': reviewer,
                'REVIEW_URL': 'https://github.com/owner/repo/pull/12#pullrequestreview-1',
                'REVIEW_STATE': 'commented', 'REVIEW_BODY': body, 'REVIEW_SUBMITTED_AT': submitted,
                'EXISTING': existing, 'API_FAIL': str(int(api_fail)), 'LOOKUP_FAIL': str(int(lookup_fail)), 'TMPDIR': str(root),
            }, capture_output=True, text=True, timeout=10)
            self.assertFalse(marker.exists(), result.stdout)
            path = root / 'mutations.jsonl'
            mutations = [json.loads(line) for line in path.read_text().splitlines()] if path.exists() else []
